    ...
    while (run)
    {
        ...
        if (sys_ready)
        {
		    ...
		    gt = 0;
                ...
                //epos4_drive_pt[1].ptOutParam->TargetVelocity=0;
                //epos4_drive_pt[2].ptOutParam->TargetVelocity=0;
                //epos4_drive_pt[3].ptOutParam->TargetVelocity=0;
    ...
    while (run)
    {
        ...
        if (inOP==TRUE)
        {
            ...
            else
            {
                ...
                rt_printf("Time = %06d.%01d, \e[32;1m fail=%ld\e[0m, ecat_T=%ld, maxT=%ld, cycle=%ld\n",
                        itime/10, itime%10, recv_fail_cnt, ethercat_time/1000, worst_time/1000, (next-previous)/1000);
                for(i=0; i<NUMOFEPOS4_DRIVE; ++i)
                {
...
int main(int argc, char *argv[])
{
    ...
    rt_task_create(&print_task, "ec_printing", 0, 70, 0 );
