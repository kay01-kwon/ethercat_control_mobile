...
int32_t wheel_des[NUMOFWHEEL_DRIVE] = {0,0,0,0};
int32_t wheel_des0[NUMOFWHEEL_DRIVE] = {0,0,0,0};
int32_t wheel_des_prev[NUMOFWHEEL_DRIVE] = {0,0,0,0};
int32_t Dwheel_des[NUMOFWHEEL_DRIVE] = {0,0,0,0};

int32_t target_vel[NUMOFWHEEL_DRIVE] = {0,0,0,0};
int32_t wheel_des_thresshold = 200;
            ...
            rt_printf("%d slaves found and configured.\n", ec_slavecount);
                ...
                for (int k=1; k<(NUMOFWHEEL_DRIVE+1); ++k)
                {
                    if (( ec_slavecount >= 1 ) && (strcmp(ec_slave[k].name,"EPOS4") == 0)) //change name for other drives
                    {
                        ...
                        //os=sizeof(ob); ob = 0x64;
                        //wkc_count=ec_SDOwrite(k,0x6085, 0x00, 
						//FALSE, os, &ob,EC_TIMEOUTRXM);
...
void EPOS_OP(void *arg)
{
    ...
    while (run)
    {
        ...
        if (sys_ready)
        {
            ...
            for (i=0; i<NUMOFWHEEL_DRIVE; ++i)
            {
				if(abs(wheel_des_prev[i]-wheel_des[i])<wheel_des_thresshold)
				{
					target_vel[i] = wheel_des[i];
					wheel_des_prev[i] = wheel_des[i];
					wheel_des0[i] = wheel_des[i];
				}
				else
				{
					Dwheel_des[i] = wheel_des[i] - wheel_des_prev[i];
					target_vel[i] = (int)wheel_des0[i] + period/T*Dwheel_des[i];
					wheel_des0[i] = target_vel[i];
				}

                epos4_drive_pt[i].ptOutParam->TargetVelocity=target_vel[i];
...
int main(int argc, char** argv)
{
    ...
    period=((double) cycle_ns)/((double) NSEC_PER_SEC);
